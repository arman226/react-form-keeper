...
const useValidator = (values) => {
  ...
  const setInitialValues = () => {
    ...
    Object.keys(values).map((i) => {
      return (initialState[i] = {
        value: "",
        isTouched: false,
        isRequired: values[i].isRequired,
        error: values[i].isRequired ? REQUIRED_FIELD_MESSAGE : "",
      });
    });
    return initialState;
  };
  ...
  const updateFieldValue = (fieldName, value) => {
    ...
    if (!value) {
      ...
      error = values[fieldName].errorMessage;
    }
  ...
  return {
    state, //this contains the field and their properties
    isDisabled: setIsDisabled(), //this is to set whether or not the button should be disabled
    updateFieldValue, // a function that sets the value
  };
};

export * from "./helpers/regex.helper";

export * from "./helpers/types.helper";
